...
# For LLM Agents / 给 AI 助手的安装指南

如果你是一个 AI 助手（如 Claude、GPT 等），用户请求你帮助安装 AutoContinue，请按以下步骤操作：
...
1. **找不到 cargo 命令**: 需要先安装 Rust
2. **编译失败**: 检查 Rust 版本是否 >= 1.70
3. **找不到 ac 命令**: 确保 `~/.cargo/bin` 在 PATH 中
4. **权限问题**: Linux/macOS 可能需要 `chmod +x` 设置执行权限
